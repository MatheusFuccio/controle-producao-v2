...
function normalizarDados(linhas){
  return linhas.map((linha, indice) => {
    const item = {
      ...
      status: (linha.status || 'SEM STATUS').toUpperCase(),
...
function aplicarFiltros(){
  ...
  estado.filtrados = estado.dados.filter(item => {
    const textoBusca = [item.os, item.cliente, item.produto, item.status, item.responsavel, item.prioridade]
      .join(' ')
      .toLowerCase();
...
function atualizarIndicadores(){
  ...
  elementos.concluidas.textContent = dados.filter(x => x.status.includes('CONCLU')).length;
  elementos.contadorFiltrado.textContent = estado.filtrados.length;
}

function renderizarTabela(){
  if(!estado.filtrados.length){
    elementos.tabela.innerHTML = `<tr><td class="empty" colspan="7">Nenhuma OS encontrada para os filtros selecionados.</td></tr>`;
    return;
  }
...
function badgeStatus(status){
  ...
  if(status.includes('CONCLU')) classe = 'badge--concluido';
...
function exportarCsvFiltrado(){
  const cabecalho = ['OS','CLIENTE','PRODUTO','LANCAMENTO','ENTREGA','STATUS','SITUACAO_PRAZO'];
